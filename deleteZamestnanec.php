...
if ($_SESSION['admin'] === 0) {
    ...
    {
        $cquery = "SELECT * FROM employee WHERE employee_id=:employeeId";
        $pdo = DB::connect();
        $cstmt = $pdo->prepare($cquery);
        $cstmt->execute(["employeeId" => $employeeId]);
    }
    ...
    if($cstmt->rowCount() == 0){
        ...
        $employee = $cstmt->fetch();

        echo "<div class='container'><h2>Vážně chceš smazat zaměstnance {$employee->surname} {$employee->name} a všechny jeho korespondující záznamy?</h2>";
        ...
        function delete()
        {
            ...
            $kquery = 'DELETE FROM `key` WHERE employee=:employeeId';
            $kstmt = $pdo->prepare($kquery);

            $kstmt->execute(["employeeId" => $employeeId]);
...
<head>
    ...
    <title>Smazat zaměstnance</title>
</head>
