...
<body>
    <form method="post" class="container">
        <h1>Nový zaměstnanec <a href=prohlizec.php class='btn btn-secondary'><i class='bi bi-caret-left-fill'></i> Zpět</a></h1><br>
...
if ($_SESSION['admin'] === 0) {
    ...
    if ($_POST) {
        ...
        if ($duplicateCheck === 0 && $nCheck === 7) {
            ...
            $query = "INSERT INTO `employee`(`name`,`surname`,job,wage,room,`login`,`password`,`admin`) VALUES ('" . $employeeName . "','" . $employeeSurname . "','" . $employeeJob . "'," . $employeeWage . "," .  $employeeRoom . ",'" . $employeeLogin . "','" . $employeePass . "'," . $employeeAdmin . ")";
            $stmt = $pdo->prepare($query);
            $stmt->execute();

            $keys_arr = explode (",", $employeeKeys);

            $empIdQuery = "SELECT `employee_id` FROM employee WHERE `login`=:employeeLogin";
            $empIdStmt = $pdo->prepare($empIdQuery);
            $empIdStmt->execute(["employeeLogin" => $employeeLogin]);
    
            $empName = $empIdStmt->fetch();
            foreach($keys_arr as $keyy)
            {
                $keysQuery = "INSERT INTO `key`(employee, room) VALUES (" . $empName->employee_id . "," . $keyy . ")";
                $keysStmt = $pdo->prepare($keysQuery);
                $keysStmt->execute();
            } 
            

            echo "<div class='container'>Nový zaměstnanec {$employeeSurname} {$employeeName} vytvořen.</div>";
        }
        $nCheck = 0;
    }
}
